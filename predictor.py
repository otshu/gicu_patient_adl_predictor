...
def get_user_input():
    ...
    new_data = {
        ...
        '入院時年齢': st.sidebar.number_input('入院時年齢', min_value=0, step=1),
        'ICD10大項目_encoded': ICD10_options[selected_ICD10],
        '入院経路_encoded': pathway_options[selected_pathway],
        'GICU入室までの日数': st.sidebar.number_input('入院からGICU入室までの日数', min_value=0, step=1),
        'stay_gicu_length_x': st.sidebar.number_input('GICU在室日数', min_value=0, step=1),
